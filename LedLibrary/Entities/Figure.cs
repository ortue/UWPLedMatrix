...
namespace LedLibrary.Entities
{
  public class Figure
  {
    ...
    //{
    //  if (Xs != null)
    //    AxisSet(Xs.Min(), Xs.Max(), null, null);

    //  if (Ys != null)
    //    AxisSet(null, (double?)null, Ys.Min(), Ys.Max());
    ...
    public void PlotScatter(double[] Xs, double[] Ys, float markerSize = 3, Color? markerColor = null)
    {
      ...
      for (int i = 0; i < points.Length; i++)
        gfxGraph.FillEllipse(new SolidBrush((Color)markerColor), points[i].X - markerSize / 2, points[i].Y - markerSize / 2, markerSize, markerSize);
